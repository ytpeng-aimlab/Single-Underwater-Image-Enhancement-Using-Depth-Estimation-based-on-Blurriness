...
DBlur = imdilate(roughBlurMap, strel('square', win));

se = strel('square',win);
marker = imdilate(DBlur,se);
FDBlur = imreconstruct(imcomplement(marker), imcomplement(DBlur));
FDBlur = imcomplement(FDBlur);

% refine blurriness map
Blur = guidedfilter_color(I, FDBlur, r, eps);
